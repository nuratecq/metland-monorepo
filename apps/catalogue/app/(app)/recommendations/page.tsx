...
export default async function RecommendationsPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      <Card><CardHeader className="font-semibold">Approval Requests ({approvals.length}) docs/PRD.md:944</CardHeader><CardContent className="space-y-2 text-sm">{approvals.length===0?<span className="text-[var(--color-on-surface-variant)]">No approvals yet</span>: approvals.map((a:unknown)=>{const aa=a as Record<string,unknown>; return <div key={String(aa.id)} className="border rounded p-2"><div>{String(aa.status)} — {String(aa.recommendation_id).slice(0,8)}</div><div className="text-xs">{String(aa.reason ?? "")}</div></div>;})}</CardContent></Card>
    </div>
  );
}
