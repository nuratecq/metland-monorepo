...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  await db.execute({
    sql: `INSERT INTO issues (id, project_id, title, description, severity, status, reported_by, assigned_to, due_date, created_at, updated_at)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    args: [iId, project_id, body.title, body.description ?? null, body.severity ?? "MEDIUM", body.status ?? "OPEN", body.reported_by ?? null, body.assigned_to ?? null, body.due_date ?? null, now, now],
  });
